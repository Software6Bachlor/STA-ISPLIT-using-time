...
class Edge:
    ...
    def solve_guard(self, expr: 'Expression', state: State, automaton: Automaton) -> Optional[list[tuple[float, float]]]:
        ...
        if isinstance(expr, BinaryExpression):
            ...
            if op == '∨':  # OR
                t_left = self.solve_guard(expr.left) # [(0,1), ()]
                t_right = self.solve_guard(expr.right) # [(2,3)]
                if t_left is None: return t_right
                if t_right is None: return t_left
                
                return interval_union(t_left, t_right)                           
            ...
            l_val, l_rate = self.evaluate_term(expr.left)
            r_val, r_rate = self.evaluate_term(expr.right)
    ...
    def evaluate_term(expr: 'Expression',state: State, automaton: Automaton) -> tuple[float, float]:
