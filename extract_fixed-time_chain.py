...
print("Scanning for FIXED TIME chain experiments...")
...
for filepath in glob.glob(os.path.join(RESULTS_DIR, "*.json")):
    ...
    if "fixed_time_chain_mc" in filename:
        method = "mc"
    elif "fixed_time_chain_restart" in filename:
        method = "restart"
    else:
        continue
...
output_filename = "extracted_fixed_time_chain.json"
