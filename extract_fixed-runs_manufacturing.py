...
data = {
    ...
    "restart": {"trials": 0, "Y_list": [], "p_estimates": [], "run_amount": 0, "sim_time": 0.0, "config_time": 0.0}
}

print("Scanning for FIXED RUNS manufacturing experiments...")
...
for filepath in glob.glob(os.path.join(RESULTS_DIR, "*.json")):
    ...
    if "fixed_runs_manufacturing_mc" in filename:
        method = "mc"
    elif "fixed_runs_manufacturing_restart" in filename:
        method = "restart"
    else:
        continue
    ...
    # Record individual file stats and accumulated times
    data[method]["run_amount"] += 1
    data[method]["p_estimates"].append(p_est)
    data[method]["sim_time"] += res.get("simElapsedSeconds", 0.0)
    ...
    # Pool RESTART data (and config time)
    elif method == "restart":
        data["restart"]["config_time"] += res.get("configElapsedSeconds", 0.0)
...
if data["mc"]["trials"] > 0:
    ...
    output_json["mc"] = {
        ...
        "total_sim_elapsed_seconds": data["mc"]["sim_time"]
    }
...
if data["restart"]["trials"] > 0:
    ...
    output_json["restart"] = {
        ...
        "total_sim_elapsed_seconds": data["restart"]["sim_time"],
        "total_config_elapsed_seconds": data["restart"]["config_time"]
    }

# 3. Write to JSON file
output_filename = "extracted_fixed_runs_manufacturing.json"
